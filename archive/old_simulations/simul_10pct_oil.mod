// 10% Oil Price Shock Simulation (sustained 4 quarters, then taper)
// Oil prices rise 10% above baseline for Q1-Q4, then decay via AR(1)
var
    y_lso pi_lso i_lso r_lso z_lso s_lso res_gap_lso res_lso res_bar_lso prem_lso g_lso
    y_zaf pi_zaf i_zaf r_zaf z_zaf s_zaf
    y_row pi_row i_row
    pi_oil pi_food rcom rcom_bar d_rcom pi_com;
...
parameters
    ...
    rho_oil rho_food rho_com rho_d_rcom
    rho_g rho_z;

// Calibration (V3 - BOP-based reserves)
...
model(linear);
    ...
    pi_zaf = lambda_1*pi_zaf(-1) + (1-lambda_1)*pi_zaf(+1) + lambda_2*y_zaf + lambda_3*(z_zaf-z_zaf(-1)) + eps_pi_zaf;
...
initval;
    ...
    pi_oil=0; pi_food=0; rcom=0; rcom_bar=0; d_rcom=0; pi_com=0;
end;

steady;

// 10% oil price shock sustained for 4 quarters then taper
// pi_oil_t = 0.70*pi_oil_{t-1} + eps_oil_t
// To hold pi_oil = 0.10 for t=1..4: eps_oil_1=0.10, eps_oil_{2..4}=0.03
// After Q4: eps_oil=0, pi_oil decays at rate 0.70 per quarter
shocks;
    var eps_oil;
    periods 1     2     3     4;
