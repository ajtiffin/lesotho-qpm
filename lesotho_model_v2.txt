// =========================================================================
// LESOTHO QPM MODEL - VERSION 2
// =========================================================================
// Based on: IMF Country Report No. 2023/269
//           SARB Working Paper WP1701 (2017) - Commodity price extension
//
// Version 2 adds:
// - Real commodity price (terms of trade) effects in South Africa IS curve
// - Based on SARB QPM model specification
// =========================================================================
...
var
    ...
    rcom_bar        // Trend component of real commodity price (NEW)
    d_rcom          // Deviation of rcom from trend (gap) - enters IS curve (NEW)
...
varexo
    ...
    eps_com         // Commodity price shock (NEW in v2)
    eps_rcom_trend  // Trend shock to real commodity price (NEW in v2)
;
...
parameters
    ...
    gamma_6         // Commodity terms of trade effect (NEW in v2)
    ...
    rho_d_rcom      // Persistence of commodity price gap (NEW in v2)
...
// -------------------------------------------------------------------------
// PARAMETER CALIBRATION (v2 - With commodity prices in SA block)
// -------------------------------------------------------------------------
// Based on IMF CR/2023/269 and SARB WP1701 (2017)
...
// South Africa IS Curve (NEW: gamma_6 from SARB WP1701)
...
rho_d_rcom  = 0.70;     // Persistence of commodity price gap (calibrated to match SARB dynamics)
...
model(linear);
    ...
    // =====================================================================
    // SOUTH AFRICA MODULE (with commodity prices - NEW in v2)
    // =====================================================================

    // 12. IS Curve with Commodity Terms of Trade (SARB WP1701 Eq. 1)
    // Now includes: y_row (foreign output) and d_rcom (commodity price GAP)
    // NOTE: Uses d_rcom (deviation from trend), not rcom (level)
    // This ensures stationarity - commodity price cycles, not permanent shifts
    y_zaf = gamma_1 * y_zaf(-1)
          + gamma_2 * y_zaf(+1)
          - gamma_3 * r_zaf
          + gamma_4 * z_zaf
          + gamma_5 * y_row
          + gamma_6 * d_rcom        // NEW: Commodity price GAP (not level)
          + eps_y_zaf;
    ...
    // =====================================================================
    // COMMODITY PRICES MODULE (NEW in v2)
    // Based on SARB WP1701: Trend-deviation decomposition for rcom
    // =====================================================================

    // 21. Real Commodity Price (Terms of Trade)
    // Definition: rcom = trend + deviation
    // This decomposition ensures stationarity in the IS curve
    rcom = rcom_bar + d_rcom;

    // 22. Trend Component (Random Walk)
    // Captures long-term structural changes in commodity markets
    // The trend is non-stationary but doesn't affect output gap directly
    rcom_bar = rcom_bar(-1) + eps_rcom_trend;

    // 23. Deviation/Gap Component (AR(1))
    // This is what enters the IS curve (d_rcom, not rcom)
    // Captures cyclical commodity price movements (gold booms, etc.)
    d_rcom = rho_d_rcom * d_rcom(-1) + eps_com;

    // 24. Nominal Commodity Price Inflation (implied identity)
    // Derived from: rcom - rcom(-1) = pi_com - pi_row
...
initval;
    ...
    pi_food     = 0;
    rcom        = 0;        // NEW in v2
    rcom_bar    = 0;        // NEW in v2: Trend component
    d_rcom      = 0;        // NEW in v2: Deviation from trend (enters IS curve)
    pi_com      = 0;        // NEW in v2
end;
...
shocks;
    ...
    var eps_com;           stderr 3.0;  // NEW in v2: Commodity price gap shock (cyclical)
    var eps_rcom_trend;    stderr 2.0;  // NEW in v2: Trend shock to real commodity price
end;
