...
// - Added oil and food price terms to SA Phillips curve (lambda_4, lambda_5)
...
var
    ...
    // Commodity prices
    pi_oil          // Oil price inflation (first difference of level)
    pi_food         // Food price inflation (first difference of level)
    poil_gap        // Oil price level gap (log deviation from trend)
    pfood_gap       // Food price level gap (log deviation from trend)
...
parameters
    ...
    lambda_4        // Oil price pass-through to SA inflation (NEW in v4)
    lambda_5        // Food price pass-through to SA inflation (NEW in v4)
    ...
    // =====================================================================
    rho_oil         // Oil price level gap persistence
    rho_food        // Food price level gap persistence
    rho_com         // Nominal commodity price persistence
    rho_d_rcom      // Persistence of commodity price gap
    kappa_oil_food  // Oil-to-food price pass-through (NEW in v4)
...
// -------------------------------------------------------------------------
// PARAMETER CALIBRATION (v4 - enriched oil/food transmission)
// -------------------------------------------------------------------------
...
kappa_oil_food = 0.05;  // Oil-to-food pass-through (NEW v4: SARB b_36=0.033, scaled for cumulative)
...
model(linear);
    ...
    // Unchanged from v3 — benefits automatically from upstream changes
    // to pi_zaf and pi_food
    pi_lso = pi_zaf
    ...
    // 13. Phillips Curve (Hybrid) — V4: added oil and food price terms
    // SARB WP1701 models fuel CPI in headline; we add reduced-form
    // oil and food pass-through to capture this channel
    ...
    // 25. Oil Price Level Gap (AR(1) on level, not inflation)
    poil_gap = rho_oil * poil_gap(-1) + eps_oil;

    // 26. Oil Price Inflation (first difference of level)
    pi_oil = poil_gap - poil_gap(-1);

    // 27. Food Price Level Gap — V4: added oil-to-food linkage
    // Energy costs are ~15-20% of food production; SARB WP1701 b_36=0.033
    // Scaled up for reduced-form cumulative effect
    pfood_gap = rho_food * pfood_gap(-1) + kappa_oil_food * poil_gap + eps_food;

    // 28. Food Price Inflation (first difference of level)
